...
type Parser interface {
	Parse(dataset Dataset, payload Payload) (string, error)
}

// BaseParser SQL:2011, SQL:2008, SQL:2003, SQL:1999, and SQL-92 Standard
type BaseParser struct {
}

func (p BaseParser) Parse(dataset Dataset, payload Payload) (string, error) {
	if dataset.Type == common.DatasetTypeSubQuery {
		ast, err := pgparser.ParseOne(dataset.SQL)
		if err != nil {
		...
		return p.parseDSLToSQL(payload, from)
	}
	table := tree.MakeUnqualifiedTableName(tree.Name(dataset.Table))
	from := tree.From{
		Tables: tree.TableExprs{&tree.AliasedTableExpr{
			Expr: &table,
		}},
...
func (p BaseParser) parseDSLToSQL(payload Payload, from tree.From) (string, error) {
	ast := &tree.Select{}
	...
	case RANGE, TEMPORAL_RANGE:
		{
			...
			if _, ok := fromValue.(int); ok {
				...
				toExpr = tree.NewStrVal(fromValue.(string))
			}
			expr.From = fromExpr
			expr.To = toExpr
			return expr
	...
	case ONE_OF:
		{
			...
			if _, ok := value.(int); ok {
				...
				for _, v := range filter.Rule.Value {
					expr := tree.NewNumVal(constant.MakeFloat64(v.(float64)), strconv.FormatFloat(v.(float64), 'f', -1, 64), false)
					exprs = append(exprs, expr)
				}
			} else {
				for _, v := range filter.Rule.Value {
